        ...
        <>
          ...
          {editing && (() => {
            ...
            return (
              <div style={{
                ...
                <button onClick={() => setEditingCell(null)} style={ghostBtn}>Done</button>
              </div>
            );
          })()}
          ...
          <div className="card2">
              ...
              <table className="scorecard">
                <thead>
                  <tr>
                    ...
                    {Array.from({ length: 9 }, (_, i) => <th key={i + 9}>{i + 10}</th>)}
                ...
                <tbody>
                  ...
                  <tr className="si-row">
                    ...
                    {course.si.slice(0, 9).map((s, i) => <td key={i}>{s}</td>)}
                    <td className="nine-sep"></td>
                    {course.si.slice(9).map((s, i) => <td key={i + 9}>{s}</td>)}
                    <td className="nine-sep"></td>
                    <td></td>
                    <td></td>
                  </tr>
                  ...
                  {groupPlayers.map((p) => {
                    ...
                    const netVsPar = totals.countTotal > 0
                      ? totals.netTotal - course.par.slice(0, totals.countF > 0 && totals.countB > 0 ? 18 : 9).reduce((a, b) => a + b, 0)
                      : null;
            ...
            <div style={{ padding: "8px 14px", borderTop: `1px solid rgba(255,255,255,0.06)`, fontSize: "11px", color: M }}>
              Tap a cell to enter score · Net = Gross − Course HCP (USGA)
            </div>
          </div>
        </>
      ) : (
...
const ghostBtn = {
  ...
  background: "transparent", color: "#5a6b5c",
